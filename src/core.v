...
module core (
    ...
    wire [31:0] id_pc;   // PC through IF/ID
    wire [31:0] ex_pc;   // PC through ID/EX
    ...
    // --- Instantiations ---
    // 3. IF instantiation - use mem_branch_target instead of mem_alu_res:
    if_stage IF (
        .clk(clk), .reset(reset), .stall_en(pc_if_id_stall), .flush_en(flush_en),
        .ex_branch_taken_in(mem_branch_taken), 
        .ex_branch_target_in(mem_branch_target),  // was mem_alu_res — THIS WAS THE ROOT CAUSE
        .pc_plus_4_out(if_pc_plus_4), .instruction_out(if_instr), .pc_out(pc_out)
    );

    // 4. IF_ID instantiation - route through flush_en instead of mem_branch_taken directly:
    ...
    id_stage ID (
        ...
        .wb_rd_in(wb_rd), 
        .wb_write_data_in(wb_result), 
        .wb_reg_write_in(wb_reg_write),
        .pc_in(id_pc),
        .operand1_out(id_ex_op1), 
        .operand2_out(id_ex_op2), 
        .immediate_out(id_ex_imm), 
        .rd_out(id_ex_rd), 
        ...
        .mul_en_out(id_ex_mul), 
        .div_en_out(id_ex_div), 
    ...
    pipeline_register_id_ex ID_EX (
        ...
        .reset(reset), 
        .stall_en(stall_en), 
        .flush_en(flush_en | id_ex_flush),
        .pc_plus_4_in(id_pc_plus_4), 
        .operand1_in(id_ex_op1), 
        .operand2_in(id_ex_op2), 
        .immediate_in(id_ex_imm), 
        .rd_in(id_ex_rd),
        ...
        .jal_in(id_ex_jal), 
        .jalr_in(id_ex_jalr), 
        .rs1_addr_in(id_ex_rs1), 
        .rs2_addr_in(id_ex_rs2), 
        .mul_en_in(id_ex_mul), 
        .div_en_in(id_ex_div), 
    ...
    // 2. EX_MEM instantiation - fix flush_en and add branch_target:
    pipeline_register_ex_mem EX_MEM (
        .clk(clk), .reset(reset), .stall_en(stall_en), .flush_en(mem_branch_taken), // was 1'b0
        .pc_plus_4_in(ex_pc_plus_4), .alu_result_in(ex_alu_res), 
        .branch_target_in(ex_branch_target),    // ADD
        .funct3_in(ex_mem_funct3), .write_data_in(ex_write_data), .rd_in(ex_rd), 
        .mem_read_in(ex_mem_read), .mem_write_in(ex_mem_write), .mem_to_reg_in(ex_mem_to_reg), .reg_write_in(ex_reg_write), 
        .jal_in(ex_jal), .jalr_in(ex_jalr), .branch_taken_in(ex_branch_taken),
        .pc_plus_4_out(mem_pc_plus_4), .alu_result_out(mem_alu_res), 
        .branch_target_out(mem_branch_target),  // ADD
        .funct3_out(mem_funct3), .write_data_out(mem_write_data), .rd_out(mem_rd), 
